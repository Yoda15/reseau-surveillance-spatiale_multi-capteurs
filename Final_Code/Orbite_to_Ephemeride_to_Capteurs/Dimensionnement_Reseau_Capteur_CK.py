...
import orbite_finale_avec_affichage

# Initialisation de Orekit
orekit.initVM()

orbite_finale_avec_affichage

# Configuration d'Orekit et téléchargement des données
# download_orekit_data_curdir()
setup_orekit_curdir()

import Ephemerides_to_CK as ECK

workbook = xlrd.open_workbook('ephemeride.xls')
ephemeris_databrute = workbook.sheet_by_index(0)  # Sélectionner la première feuille
cartesian_orbits = ECK.ephemeris_to_cartesian(ephemeris_databrute)
keplerian_orbits = ECK.cartesian_to_keplerian(cartesian_orbits)
propagator = keplerian_orbits[1][0]
...
for k in range(2):
    ...
    for day_of_week in range(num_days):
        ...
        date_comp = DateComponents(observation_start_time.year, observation_start_time.month,
                                   observation_start_time.day)
            ...
            if times:
                ...
                for time in times[1:]:
                    if (time.durationFrom(end_time)) > (timedelta(minutes=2).total_seconds()):
                        all_pass_times.append(
                            (absolute_date_to_datetime(start_time), absolute_date_to_datetime(end_time),
    ...
    def display_pass_times_fixed_periods(all_pass_times, period_duration):
        ...
        for day_of_week in range(num_days):
            ...
            start_of_day = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(
                days=day_of_week)
            ...
            while period_start_time < end_of_day:
                ...
                print(
                    f"Nombre de capteurs disponibles : {len(period_observers)} ({', '.join(period_observers.keys())})")
                for observer_name, times in period_observers.items():
                    for start, end in times:
                        duration = end - start
                        print(
                            f"{observer_name}: Début : {start.isoformat()} - Fin : {end.isoformat()} - Durée : {duration}")
                print()
    ...
    def find_minimal_sensors_per_day(all_pass_times, period_duration):
        ...
        for day_of_week in range(num_days):
            ...
            for num_sensors in range(1,
                                     len(periods) + 1):  # Change here to limit the number of sensors to the number of periods
    ...
    # Créer le graphique
    if k == 0:
        axs[0].plot(time_deltas, min_sensors, label = 'AvantManoeuvre', marker='o')
        axs[0].set_title('Nombre minimum de capteurs en fonction de la durée entre deux observations')
        axs[0].set_xlabel('Durée entre deux observations (heures)')
        axs[0].set_ylabel('Nombre minimum de capteurs')
        axs[0].grid(True)
    else :
        axs[1].plot(time_deltas, min_sensors, label='ApresManoeuvre', marker='o')
        axs[1].set_title('Nombre minimum de capteurs en fonction de la durée entre deux observations')
        axs[1].set_xlabel('Durée entre deux observations (heures)')
        axs[1].set_ylabel('Nombre minimum de capteurs')
        axs[1].grid(True)
    ligne_nb = len(keplerian_orbits)
    # print(ligne_nb)
    propagator = keplerian_orbits[ligne_nb-1][0]
print("\n----------------------------------------------------------------------FIN----------------------------------------------------")
plt.show()
